...
void printMST(vector<Edge> MST, string str)
{
    double minimumWeight = 0;
    cout << "List of edges selected by " << str <<":{ ";
    for(auto edge : MST){
...
vector<Edge> findPrimMST(double** adj, int n)
{
    // int parent[n];
    // double key[n];
    bool visited[n];
...
int main()
{
    ...
    printMST(findPrimMST(adj, n),"prim");

    printMST(findKruskalMST(edgeListKruskal, n),"kruskal");
}