...
list<Node*> consolidateBinomialHeap(list<Node*> *bh){
    ...
    while(curr_tree != (*bh).end()){
        // cout << (*prev_tree)->getValue() << endl;
        // cout << (*curr_tree)->getValue() << endl;
        // cout << (*next_tree)->getValue() << endl;
        // cout << "-------------------" << endl;
        // printHeap(bh);
        ...
        if((*next_tree)->getValue() != (*bh).back()->getValue()){
            ...
            if((*temp) != nullptr && (*curr_tree)->getDegree() == (*temp)->getDegree()){
                prev_tree = curr_tree;
                curr_tree = next_tree;
                next_tree++;
            }
