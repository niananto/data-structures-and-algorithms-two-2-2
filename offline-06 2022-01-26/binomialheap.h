...
list<Node*> consolidateBinomialHeap(list<Node*> *bh){
    ...
    if((*bh).size() == 2){
        ...
        if((*curr_tree)->getValue() > (*next_tree)->getValue()){
            *curr_tree = mergeBinomialTrees((*curr_tree), (*next_tree));
            ...
            list<Node*>::iterator temp = curr_tree;
            *curr_tree = mergeBinomialTrees((*next_tree), (*curr_tree));
            (*bh).erase(temp);
        }
    ...
    // while(next_tree != (*bh).end()){
        ...
        // move ahead
        if((*next_tree)->getValue() != (*bh).back()->getValue()){
            list<Node*>::iterator temp = next_tree;
            temp++;

            if(temp != (*bh).end() && (*curr_tree)->getDegree() == (*temp)->getDegree()){
                prev_tree = curr_tree;
                curr_tree = next_tree;
                next_tree++;
            }
        ...
        if((*curr_tree)->getValue() >= (*next_tree)->getValue()){
            ...
            mergeBinomialTrees((*curr_tree), (*next_tree));
            (*bh).erase(next_tree);
        ...
        // make curr a child of next
            ...
            mergeBinomialTrees((*next_tree), (*curr_tree));
                // cout << "Here" << endl;
                // printHeap(bh);
    ...
    // }
    // return (*bh);
    return consolidateBinomialHeap(bh);
}
...
list<Node*> extractMax(list<Node*> *bh){
    ...
    // cout << "new heap 1" << endl;
    // printHeap(&newHeap1);
    // cout << "new heap 2" << endl;
    // printHeap(&newHeap2);
